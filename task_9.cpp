...
void task_9(){
    ...
    TFile outfile("ofile_task8.root", "update");
    ...
    graph1->SetTitle("vs #theta");

    auto graph2 = new TGraphErrors();
    graph2->SetTitle("vs on #phi");
    ...
    my_graph->SetTitle("number of candidates for #pi^{0} vs azimuthal and polar angle;angle [degree];candidates");
    my_graph->Add(graph1);
    my_graph->Add(graph2);
    my_graph->Draw("ap");
